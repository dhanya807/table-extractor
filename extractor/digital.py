...
def _detect_table_structure(df: pd.DataFrame) -> tuple[bool, int, List[str]]:
    """Automatically detect if this is a structured table and find the header row using structural analysis."""
    ...
    # Find the best header row using structural analysis
    best_header_idx = -1
    best_header_score = 0
    best_headers = []
    
    # Analyze the data below each potential header to determine which is most likely
    for idx, headers in potential_headers:
        # Get data rows below this potential header
        data_rows = []
        for i in range(idx + 1, len(df)):
            row = df.iloc[i]
            data_row = [str(c).strip() if pd.notna(c) else "" for c in row]
            if any(c for c in data_row):  # Non-empty row
        ...
        score = _calculate_header_score(headers, data_rows)
...
def _calculate_header_score(headers: List[str], data_rows: List[List[str]]) -> float:
    """Calculate how likely a row is to be a header based on structural analysis."""
    ...
    # 1. Uniqueness score - headers should be unique
    unique_headers = len(set(normalized_headers))
    uniqueness_score = unique_headers / len(normalized_headers) if normalized_headers else 0
    
    # 2. Length score - headers should be reasonably sized
    avg_header_length = sum(len(h) for h in normalized_headers) / len(normalized_headers) if normalized_headers else 0
    length_score = min(avg_header_length / 20.0, 1.0)  # Normalize to 0-1, prefer 20+ chars
    
    # 3. Data consistency score - check if data below looks structured
    consistency_score = _analyze_data_consistency(headers, data_rows)
    
    # 4. Position score - headers are usually near the top
    # This is handled by the calling function which processes rows in order
    
    # 5. Content type score - headers should be descriptive text, not data
    content_score = _analyze_header_content_types(normalized_headers)
...
def _analyze_data_consistency(headers: List[str], data_rows: List[List[str]]) -> float:
    """Analyze how well the data rows align with the potential headers."""
    ...
    for col_idx in range(min(len(headers), max(len(row) for row in data_rows))):
        column_data = []
        for row in data_rows:
            if col_idx < len(row) and row[col_idx].strip():
                column_data.append(row[col_idx])
        
        if len(column_data) >= 2:  # At least 2 non-empty values
            # Check if column has consistent data type
            data_types = [_classify_cell_content(cell) for cell in column_data]
            if len(set(data_types)) <= 2:  # Most cells have similar types
                consistent_columns += 1
    
    return consistent_columns / total_columns if total_columns > 0 else 0.0


def _analyze_header_content_types(headers: List[str]) -> float:
    """Analyze if headers look like descriptive labels rather than data."""
    
    if not headers:
        return 0.0
    
    good_header_indicators = 0
    total_headers = len(headers)
    
    for header in headers:
        # Good indicators for headers
        if len(header) >= 3:  # Reasonable length
            if not header.replace('.', '').replace(',', '').replace('-', '').isdigit():  # Not purely numeric
                if not any(char.isdigit() for char in header[:3]):  # Doesn't start with numbers
                    if len(header.split()) <= 4:  # Not too many words
                        good_header_indicators += 1
    
    return good_header_indicators / total_headers if total_headers > 0 else 0.0


def _classify_cell_content(cell: str) -> str:
    """Classify the content type of a cell."""
    cell = cell.strip()
    ...
    # Check if it's a code (short, mostly uppercase)
    if len(cell) <= 15 and any(char.isupper() for char in cell) and not any(char.islower() for char in cell):
        return "code"
    
    # Check if it's a date
    if re.match(r'\d{1,2}[/-]\d{1,2}[/-]\d{2,4}', cell) or re.match(r'\d{4}[/-]\d{1,2}[/-]\d{1,2}', cell):
        return "date"
...
def _detect_row_relationships(headers: List[str], rows: List[List[str]]) -> List[List[List[str]]]:
    ...
    def analyze_row_pattern(row: List[str]) -> dict:
        ...
        for i, cell in enumerate(row):
            ...
            # Detect content types using the same classification as header analysis
            content_type = _classify_cell_content(cell)
            content_types.append((content_type, i))
        
        return {
            'non_empty_count': len(non_empty),
            'content_types': content_types,
            'has_numbers': any(t[0] in ['float', 'integer'] for t in content_types),
            'has_codes': any(t[0] == 'code' for t in content_types),
            'has_dates': any(t[0] == 'date' for t in content_types)
        }
